                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            <?php
                                ...
                                if (isset($_GET['id'])) {
                                    $idDeposito = secure_data($_GET['id']);

                                    // Verificar si el depósito está asociado a la tabla producto
                                    $consultaVerificarProducto = "SELECT * FROM producto WHERE idDeposito = :idDeposito";
                                    $stmtVerificarProducto = $connectDB->prepare($consultaVerificarProducto);
                                    $stmtVerificarProducto->bindValue(':idDeposito', $idDeposito);
                                    $stmtVerificarProducto->execute();

                                    // Si existen registros asociados en la tabla producto, mostrar error
                                    if ($stmtVerificarProducto->rowCount() > 0) {
                                        echo "<script>
                                                Swal.fire({
                                                    icon: 'error',
                                                    title: 'Error al borrar',
                                                    text: 'El Depósito está asociado a la tabla Producto. No se puede eliminar.',
                                        ...
                                        $stmtBorrarDeposito = $connectDB->prepare('DELETE FROM deposito WHERE idDeposito = :idDeposito');
                                        $stmtBorrarDeposito->bindValue(':idDeposito', $idDeposito);
                                        $stmtBorrarDeposito->execute();
                                    ...
                                    echo "<script>
                                            Swal.fire({
                                                icon: 'error',
                                                title: 'Error al borrar',
                                                text: 'Hubo un problema al intentar borrar El Depósito',
                                                showConfirmButton: false,
                                                timer: 1500
                                            }).then(function() {
                                                window.location.href = 'indexDeposito.php';
