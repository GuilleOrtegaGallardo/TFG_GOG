            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <div class="container-fluid px-4">
                        <h4 class="mt-4">Crear Deposito</h4>
                            ...
                            <div class="card-body">
                                ...
                                <?php
                                    ...
                                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                                        ...
                                        if (
                                            isset($_POST['idDeposito']) && isset($_POST['descripcion']) &&
                                            isset($_POST['limiteOcupacion']) && isset($_POST['cantidadOcupada']) &&
                                            isset($_POST['porcentajeOcupacion']) && isset($_POST['ubicacion'])
                                        ) {
                                            // Recoger los datos del formulario
                                            $idDeposito = secure_data($_POST['idDeposito']);
                                            $descripcion = secure_data($_POST['descripcion']);
                                            $limiteOcupacion = secure_data($_POST['limiteOcupacion']);
                                            $cantidadOcupada = secure_data($_POST['cantidadOcupada']);
                                            $porcentajeOcupacion = secure_data($_POST['porcentajeOcupacion']);
                                            $ubicacion = secure_data($_POST['ubicacion']);

                                            // Verificar si el idDeposito ya existe en la base de datos
                                            $stmtCheck = $connectDB->prepare('SELECT idDeposito FROM deposito WHERE idDeposito = :idDeposito');
                                            $stmtCheck->bindParam(':idDeposito', $idDeposito);
                                            $stmtCheck->execute();

                                            if ($stmtCheck->rowCount() > 0) {
                                                // Mostrar SweetAlert2 de error si el idDeposito ya existe
                                                echo "<script>
                                                        Swal.fire({
                                                            icon: 'error',
                                                            title: 'Error al insertar',
                                                            text: 'El ID de Depósito ya existe en la base de datos',
                                                ...
                                                $sql = 'INSERT INTO deposito (idDeposito, descripcion, limiteOcupacion, cantidadOcupada, porcentajeOcupacion, ubicacion) 
                                                        VALUES (:idDeposito, :descripcion, :limiteOcupacion, :cantidadOcupada, :porcentajeOcupacion, :ubicacion)';
                                                ...
                                                echo "<script>
                                                        Swal.fire({
                                                            ...
                                                            window.location.href = 'indexDeposito.php';
                                                        });
                                                    </script>";
                                            }
                                        } else {
                                ...
                                <form action="nuevoDeposito.php" method="post" enctype="multipart/form-data" onsubmit="return validarFormularioDeposito();">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="idDeposito" id="idDeposito" aria-describedby="helpId" placeholder="ID DEPOSITO"
                                            value="<?php echo obtenerSiguienteIdDeposito(); ?>" readonly>
                                    </div>

                                    <?php
                                    function obtenerSiguienteIdDeposito()
                                    {
                                        global $connectDB;

                                        try {
                                            // Obtener el último idDeposito de la tabla deposito
                                            $consultaUltimoId = "SELECT MAX(idDeposito) as ultimoId FROM deposito";
                                            ...
                                            $siguienteIdDeposito = $parteNumerica + 1;

                                            // Formatear el nuevo ID con ceros a la izquierda y el prefijo "DEP"
                                            $nuevoIdDeposito = sprintf("DEP%04d", $siguienteIdDeposito);

                                            return $nuevoIdDeposito;
                                    ...
                                    <input type="text"
                                        class="form-control" name="limiteOcupacion" id="limiteOcupacion" aria-describedby="helpId" placeholder="LIMITE DE OCUPACION" required>
                                    </div>

                                    <div class="mb-3">
                                    <label for="cantidadOcupada" class="form-label">CANTIDAD OCUPADA</label>
                                    <input type="text"
                                        class="form-control" name="cantidadOcupada" id="cantidadOcupada" aria-describedby="helpId" placeholder="CANTIDAD OCUPADA" required>
                                    </div>

                                    <div class="mb-3">
                                    <label for="porcentajeOcupacion" class="form-label">PORCENTAJE OCUPACION</label>
                                    <input type="text"
                                        class="form-control" name="porcentajeOcupacion" id="porcentajeOcupacion" aria-describedby="helpId" placeholder="PORCENTAJE OCUPACION" required>
                                    </div>

                                    <div class="mb-3">
                                    <label for="ubicacion" class="form-label">UBICACION</label>
                                    <input type="text"
                                        class="form-control" name="ubicacion" id="ubicacion" aria-describedby="helpId" placeholder="" required>
                                    </div>
                                    ...
                                    <script>
                                        function validarFormularioDeposito() {
                                            ...
                                            var porcentajeOcupacion = document.getElementById('porcentajeOcupacion').value;

                                            // Expresiones regulares para validar los formatos
                                            var formatoDeposito = /^DEP\d{4}$/.test(idDeposito);
                                            var formatoPorcentaje = /^(100(\.00?)?|\d{1,2}(\.\d{1,2})?)$/.test(porcentajeOcupacion);
                                            ...
                                            if (!formatoDeposito) {
                                                Swal.fire({
                                                    ...
                                                    text: 'El ID de Depósito debe seguir el formato DEP seguido de cuatro números (ej: DEP0000)'
                                                });
                                                return false;
                                            }
                                            ...
                                            if (!formatoPorcentaje) {
                                                Swal.fire({
                                                    icon: 'error',
                                                    title: 'Error de validación',
                                                    text: 'El Porcentaje de Ocupación debe ser un valor válido (ej: 20.00, 100.00)'
                                                });
                                                return false;
                                            }
