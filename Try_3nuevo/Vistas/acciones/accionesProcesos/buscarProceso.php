            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <div class="container-fluid px-4">
                        <h4 class="mt-4">Buscar proceso</h4>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <?php
                                    ...
                                    $idReactor = $_POST['idReactor'] ?? '';
                                    $idUsuario = $_POST['idUsuario'] ?? '';
                                    $idLineaProduccion = $_POST['idLineaProduccion'] ?? '';
                                    ...
                                    if (!empty($idProceso) || !empty($idProducto) || !empty($idReactor) || !empty($idUsuario) ||
                                        !empty($idLineaProduccion) || !empty($fechaProceso) || !empty($horaInicio) || !empty($horaFin) ||
                                        !empty($temperaturaMax) || !empty($temperaturaMin)) {

                                        // Construir la consulta SQL base
                                        $sql = 'SELECT * FROM proceso WHERE 1=1';
                                        ...
                                        if (!empty($idLineaProduccion)) {
                                            $sql .= ' AND idLineaProduccion = :idLineaProduccion';
                                        }

                                        if (!empty($fechaProceso)) {
                                            $sql .= ' AND fechaProceso = :fechaProceso';
                                        }

                                        if (!empty($horaInicio)) {
                                            $sql .= ' AND horaInicio = :horaInicio';
                                        }

                                        if (!empty($horaFin)) {
                                            $sql .= ' AND horaFin = :horaFin';
                                        }

                                        if (!empty($temperaturaMax)) {
                                            $sql .= ' AND temperaturaMax = :temperaturaMax';
                                        }

                                        if (!empty($temperaturaMin)) {
                                            $sql .= ' AND temperaturaMin = :temperaturaMin';
                                        }
                                        ...
                                        if (!empty($resultados)) {
                                            ...
                                            foreach ($resultados as $fila) {
                                                echo '<tr>
                                                        ...
                                                        <td>' . $fila['idReactor'] . '</td>
                                                        <td>' . $fila['idUsuario'] . '</td>
                                                        <td>' . $fila['idLineaProduccion'] . '</td>
                                                        <td>' . $fila['fechaProceso'] . '</td>
                                                        <td>' . $fila['horaInicio'] . '</td>
                                                        <td>' . $fila['horaFin'] . '</td>
                                                        <td>' . $fila['temperaturaMax'] . '</td>
                                                        <td>' . $fila['temperaturaMin'] . '</td>
                                                        <td>
                                                            <a name="editar" id=""  href="editarProceso.php?id=' . $fila['idProceso'] . '"><i class="fa-solid fa-pen-to-square"></i></a>
                                                            <a name="eliminar" id=""  href="deleteProceso.php?id=' . $fila['idProceso'] . '"><i class="fa-solid fa-trash"></i></a>
                                                        </td>
                                                    </tr>';
                                            }
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <form action="buscarProceso.php" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="idUsuario" id="idUsuario" aria-describedby="helpId" placeholder="ID USUARIO">
                                    </div>

                                    <div class="mb-3">
                                        <label for="idLinea" class="form-label">ID LINEA</label>
                                        <input type="text" class="form-control" name="idLinea" id="idLinea" aria-describedby="helpId" placeholder="ID LINEA">
                                    </div>

                                    <div class="mb-3">
                                        <label for="fechaProceso" class="form-label">FECHA PROCESO</label>
                                        <input type="date" class="form-control" name="fechaProceso" id="fechaProceso" aria-describedby="helpId" placeholder="FECHA PROCESO">
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        <label for="temperaturaMax" class="form-label">TEMPERATURA MAX</label>
                                        <input type="number" class="form-control" name="temperaturaMax" id="temperaturaMax" aria-describedby="helpId" placeholder="TEMPERATURA MAX">
                                    </div>

                                    <div class="mb-3">
                                        <label for="temperaturaMin" class="form-label">TEMPERATURA MIN</label>
                                        <input type="number" class="form-control" name="temperaturaMin" id="temperaturaMin" aria-describedby="helpId" placeholder="TEMPERATURA MIN">
                                    </div>

                                    <button type="submit" class="btn btn-success">Realizar Búsqueda</button>
