                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            <?php
                                ...
                                if (isset($_GET['id'])) {
                                    $idProceso = secure_data($_GET['id']);

                                    // Verificar si el proceso está asociado a la tabla bolsa
                                    $consultaVerificarBolsa = "SELECT * FROM bolsa WHERE idProceso = :idProceso";
                                    $stmtVerificarBolsa = $connectDB->prepare($consultaVerificarBolsa);
                                    $stmtVerificarBolsa->bindValue(':idProceso', $idProceso);
                                    $stmtVerificarBolsa->execute();

                                    // Si existen registros asociados en la tabla bolsa, mostrar error
                                    if ($stmtVerificarBolsa->rowCount() > 0) {
                                        echo "<script>
                                                Swal.fire({
                                                    icon: 'error',
                                                    title: 'Error al borrar',
                                                    text: 'El Proceso está asociado a la tabla Bolsa. No se puede eliminar.',
                                        ...
                                        $stmtBorrarProceso = $connectDB->prepare('DELETE FROM proceso WHERE idProceso = :idProceso');
                                        $stmtBorrarProceso->bindValue(':idProceso', $idProceso);
                                        $stmtBorrarProceso->execute();
                                    ...
                                    echo "<script>
                                            Swal.fire({
                                                icon: 'error',
                                                title: 'Error al borrar',
                                                text: 'Hubo un problema al intentar borrar el Proceso',
                                                showConfirmButton: false,
                                                timer: 1500
                                            }).then(function() {
                                                window.location.href = 'indexProceso.php';
