                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            <?php
                                ...
                                if (isset($_GET['id'])) {
                                    $idProducto = secure_data($_GET['id']);

                                    // Verificar si el producto está asociado a la tabla proceso
                                    $consultaVerificarProceso = "SELECT * FROM proceso WHERE idProducto = :idProducto";
                                    $stmtVerificarProceso = $connectDB->prepare($consultaVerificarProceso);
                                    $stmtVerificarProceso->bindValue(':idProducto', $idProducto);
                                    $stmtVerificarProceso->execute();

                                    // Verificar si el producto está asociado a la tabla pedido
                                    $consultaVerificarPedido = "SELECT * FROM pedido WHERE idProducto = :idProducto";
                                    $stmtVerificarPedido = $connectDB->prepare($consultaVerificarPedido);
                                    $stmtVerificarPedido->bindValue(':idProducto', $idProducto);
                                    $stmtVerificarPedido->execute();

                                    // Verificar si el producto está asociado a la tabla bolsa
                                    $consultaVerificarBolsa = "SELECT * FROM bolsa WHERE idProducto = :idProducto";
                                    $stmtVerificarBolsa = $connectDB->prepare($consultaVerificarBolsa);
                                    $stmtVerificarBolsa->bindValue(':idProducto', $idProducto);
                                    $stmtVerificarBolsa->execute();

                                    // Si existen registros asociados en alguna de las tablas, mostrar error
                                    if ($stmtVerificarProceso->rowCount() > 0 || $stmtVerificarPedido->rowCount() > 0 || $stmtVerificarBolsa->rowCount() > 0) {
                                        echo "<script>
                                                Swal.fire({
                                                    icon: 'error',
                                                    title: 'Error al borrar',
                                                    text: 'El Producto está asociado a otras tablas. No se puede eliminar.',
                                        ...
                                        // No hay registros asociados, realizar el borrado
                                        $stmtBorrarProducto = $connectDB->prepare('DELETE FROM producto WHERE idProducto = :idProducto');
                                        $stmtBorrarProducto->bindValue(':idProducto', $idProducto);
                                        $stmtBorrarProducto->execute();
                                    ...
                                    echo "<script>
                                            Swal.fire({
                                                icon: 'error',
                                                title: 'Error al borrar',
                                                text: 'Hubo un problema al intentar borrar el Producto',
                                                showConfirmButton: false,
                                                timer: 1500
                                            }).then(function() {
                                                window.location.href = 'indexProducto.php';
