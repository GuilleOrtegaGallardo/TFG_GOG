...
<?php 
    ...
    // // si no existe idUsuarioLogin entonces ...
    ...
    // // Para loggin de usuarios no admins
    // $tipoUsuarioLogin = $_SESSION['tipoUsuarioLogin'];
...
<?php 
    ...
    // session_start();
    // // require_once("../../Conexion_DB/conexionDB.php");
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            <?php
                                ...
                                if (isset($_GET['id'])) {
                                    $idProveedor = secure_data($_GET['id']);

                                    // Verificar si el proveedor está asociado a la tabla pedido
                                    $consultaVerificarPedido = "SELECT * FROM pedido WHERE idProveedor = :idProveedor";
                                    $stmtVerificarPedido = $connectDB->prepare($consultaVerificarPedido);
                                    $stmtVerificarPedido->bindValue(':idProveedor', $idProveedor);
                                    $stmtVerificarPedido->execute();

                                    // Si existen registros asociados en la tabla pedido, mostrar error
                                    if ($stmtVerificarPedido->rowCount() > 0) {
                                        echo "<script>
                                                Swal.fire({
                                                    icon: 'error',
                                                    title: 'Error al borrar',
                                                    text: 'El Proveedor está asociado a la tabla Pedido. No se puede eliminar.',
                                        ...
                                        $stmtBorrarProveedor = $connectDB->prepare('DELETE FROM proveedor WHERE idProveedor = :idProveedor');
                                        $stmtBorrarProveedor->bindValue(':idProveedor', $idProveedor);
                                        $stmtBorrarProveedor->execute();
                                    ...
                                    echo "<script>
                                            Swal.fire({
                                                icon: 'error',
                                                title: 'Error al borrar',
                                                text: 'Hubo un problema al intentar borrar el Proveedor',
                                                showConfirmButton: false,
                                                timer: 1500
                                            }).then(function() {
                                                window.location.href = 'indexProveedor.php';
