                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            <?php
                                ...
                                if (isset($_GET['id'])) {
                                    $idReactor = secure_data($_GET['id']);

                                    // Verificar si el reactor está asociado a la tabla proceso
                                    $consultaVerificarProceso = "SELECT * FROM proceso WHERE idReactor = :idReactor";
                                    $stmtVerificarProceso = $connectDB->prepare($consultaVerificarProceso);
                                    $stmtVerificarProceso->bindValue(':idReactor', $idReactor);
                                    ...
                                    if ($stmtVerificarProceso->rowCount() > 0) {
                                        echo "<script>
                                                Swal.fire({
                                                    ...
                                                    text: 'El Reactor está asociado a la tabla Proceso. No se puede eliminar.',
                                        ...
                                        $stmtBorrarReactor = $connectDB->prepare('DELETE FROM reactor WHERE idReactor = :idReactor');
                                        $stmtBorrarReactor->bindValue(':idReactor', $idReactor);
                                        $stmtBorrarReactor->execute();
                                    ...
                                    echo "<script>
                                            Swal.fire({
                                                icon: 'error',
                                                title: 'Error al borrar',
                                                text: 'Hubo un problema al intentar borrar el Reactor',
                                                showConfirmButton: false,
                                                timer: 1500
                                            }).then(function() {
                                                window.location.href = 'indexReactor.php';
