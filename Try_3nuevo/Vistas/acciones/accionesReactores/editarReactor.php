            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <br/>
                            ...
                            <?php
                                ...
                                if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['Enviar'])) {
                                    ...
                                    $fechaInstalacion = secure_data($_GET['fechaInstalacion']);
                                    $fechaMantenimiento = secure_data($_GET['fechaMantenimiento']);

                                    // Validar abreviatura con expresión regular
                                    if (!preg_match('/^R\d+$/', $abreviatura)) {
                                        echo "<script>
                                                Swal.fire({
                                                    icon: 'error',
                                                    title: 'Error de validación',
                                                    text: 'La abreviatura debe ser R seguida de un número (Ej. R1, R2, R3, ...)',
                                                    showConfirmButton: false,
                                                    timer: 1500
                                                });
                                            </script>";
                                    } elseif (empty($nombre) || empty($abreviatura) || empty($descripcion) || empty($fechaInstalacion) || empty($fechaMantenimiento)) {
                                    ...
                                    } elseif (strtotime($fechaInstalacion) > strtotime($fechaMantenimiento)) {
                                        // Validar que fechaInstalacion sea anterior o igual a fechaMantenimiento
                                        echo "<script>
                                                Swal.fire({
                                                    icon: 'error',
                                                    title: 'Error de validación',
                                                    text: 'La fecha de instalación debe ser anterior o igual a la fecha de mantenimiento',
                                                    showConfirmButton: false,
                                                    timer: 1500
                                                });
                                            </script>";
                                        ...
                                        $sql = 'UPDATE reactor SET 
                                                ...
                                                fechaInstalacion = COALESCE(:fechaInstalacion, fechaInstalacion),
                                                fechaMantenimiento = COALESCE(:fechaMantenimiento, fechaMantenimiento)
                                                WHERE idReactor = :idReactor';
                                ...
                                $stmt = $connectDB->prepare('SELECT * FROM reactor WHERE idReactor = :idReactor');
                                $stmt->bindParam(':idReactor', $idReactor);
                                $stmt->execute();
                                $reactorActual = $stmt->fetch(PDO::FETCH_ASSOC);

                                if (!$reactorActual) {
                                    echo "Reactor no encontrado.";
                                    exit();
                                }
                        ...
                        <div class="container-fluid px-4">
                        <h4 class="mt-4">Editar reactor</h4>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <table id="datatablesSimple">
                                    ...
                                    <tr>
                                        <td><?php echo $reactorActual['idReactor']; ?></td>
                                        <td><?php echo $reactorActual['nombre']; ?></td>
                                        <td><?php echo $reactorActual['abreviatura']; ?></td>
                                        <td><?php echo $reactorActual['descripcion']; ?></td>
                                        <td><?php echo $reactorActual['fechaInstalacion']; ?></td>
                                        <td><?php echo $reactorActual['fechaMantenimiento']; ?></td>

                                        <td>
                                            <?php
                                            echo '<a href="editarReactor.php?id=' . $reactorActual['idReactor'] . '">';
                                            echo '<i class="fa-solid fa-pen-to-square"></i>';
                                            echo '</a>';
                                            echo ' | ';
                                            echo '<a href="deleteReactor.php?id=' . $reactorActual['idReactor'] . '">';
                        ...
                        <div class="card mb-4">
                            ...
                            <h2>Formulario de Edición de Reactor</h2>

                                <form action="" method="get">

                                    <div class="mb-3">
                                        <label for="idReactor" class="form-label">ID REACTOR</label>
                                        <input class="form-control col-4" type="text" name="id" value="<?php echo $reactorActual['idReactor']; ?>" readonly>
                                    </div>

                                    <div class="mb-3">
                                        <label for="nombre">Nombre:</label>
                                        <input class="form-control col-4" type="text" id="nombre" name="nombre" value="<?php echo $reactorActual['nombre']; ?>">
                                    </div>

                                    <div class="mb-3">
                                        <label for="abreviatura">Abreviatura:</label>
                                        <input class="form-control col-4" type="text" id="abreviatura" name="abreviatura" value="<?php echo $reactorActual['abreviatura']; ?>">
                                    </div>

                                    <div class="mb-3">
                                        <label for="descripcion">Descripción:</label>
                                        <input class="form-control col-4" type="text" id="descripcion" name="descripcion" value="<?php echo $reactorActual['descripcion']; ?>">
                                    </div>

                                    <div class="mb-3">
                                        <label for="fechaInstalacion">Fecha de Instalación:</label>
                                        <input class="form-control col-4" type="date" id="fechaInstalacion" name="fechaInstalacion" value="<?php echo $reactorActual['fechaInstalacion']; ?>">
                                    </div>

                                    <div class="mb-3">
                                        <label for="fechaMantenimiento">Fecha de Mantenimiento:</label>
                                        <input class="form-control col-4" type="date" id="fechaMantenimiento" name="fechaMantenimiento" value="<?php echo $reactorActual['fechaMantenimiento']; ?>">
                                    </div>

                                    

                                    <input type="submit" class="btn btn-success" name="Enviar" value="Actualizar">
                                    <a href="indexReactor.php"><button class="btn btn-primary" type="button">Volver</button></a>
