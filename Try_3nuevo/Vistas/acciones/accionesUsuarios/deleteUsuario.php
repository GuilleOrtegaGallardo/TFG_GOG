                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card-body">
                            <?php
                                ...
                                // if(isset($_GET['id'])){
                                //     $id_usuario = secure_data($_GET['id']);
                                    

                                //     $stmt = $connectDB->prepare('DELETE FROM usuarios WHERE idUsuario=:id');
                                ...
                                //         text: 'Hubo un problema al intentar borrar el Usuario',
                                //         showConfirmButton: false,
                                //         timer: 1500
                                //     }).then(function() {
                                //         window.location.href = 'indexUsuario.php';
...
if (isset($_GET['id'])) {
    $idUsuario = secure_data($_GET['id']);

    // Verificar si el usuario está asociado a la tabla proceso
    $consultaVerificarProceso = "SELECT * FROM proceso WHERE idUsuario = :idUsuario";
    $stmtVerificarProceso = $connectDB->prepare($consultaVerificarProceso);
    $stmtVerificarProceso->bindValue(':idUsuario', $idUsuario);
    $stmtVerificarProceso->execute();

    // Verificar si el usuario está asociado a la tabla pedido
    $consultaVerificarPedido = "SELECT * FROM pedido WHERE idUsuario = :idUsuario";
    $stmtVerificarPedido = $connectDB->prepare($consultaVerificarPedido);
    $stmtVerificarPedido->bindValue(':idUsuario', $idUsuario);
    $stmtVerificarPedido->execute();

    // Si existen registros asociados en alguna de las tablas, mostrar error
    if ($stmtVerificarProceso->rowCount() > 0 || $stmtVerificarPedido->rowCount() > 0) {
        echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Error al borrar',
                    text: 'El Usuario está asociado a las tablas Proceso o Pedido. No se puede eliminar.',
                    showConfirmButton: false,
                    timer: 2500
                }).then(function() {
                    window.location.href = 'indexUsuario.php';
        ...
        $stmtBorrarUsuario = $connectDB->prepare('DELETE FROM usuarios WHERE idUsuario = :idUsuario');
        $stmtBorrarUsuario->bindValue(':idUsuario', $idUsuario);
