                    ...
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            <?php
                                ...
                                if (isset($_GET['id'])) {
                                    $idLineaProduccion = secure_data($_GET['id']);

                                    // Verificar si la línea de producción está asociada a la tabla proceso
                                    $consultaVerificarProceso = "SELECT * FROM proceso WHERE idLineaProduccion = :idLineaProduccion";
                                    $stmtVerificarProceso = $connectDB->prepare($consultaVerificarProceso);
                                    $stmtVerificarProceso->bindValue(':idLineaProduccion', $idLineaProduccion);
                                    $stmtVerificarProceso->execute();

                                    // Si existen registros asociados en la tabla proceso, mostrar error
                                    if ($stmtVerificarProceso->rowCount() > 0) {
                                        echo "<script>
                                                Swal.fire({
                                                    icon: 'error',
                                                    title: 'Error al borrar',
                                                    text: 'La Línea de Producción está asociada a la tabla Proceso. No se puede eliminar.',
                                        ...
                                        $stmtBorrarLineaProduccion = $connectDB->prepare('DELETE FROM lineaproduccion WHERE idLineaProduccion = :idLineaProduccion');
                                        $stmtBorrarLineaProduccion->bindValue(':idLineaProduccion', $idLineaProduccion);
                                        $stmtBorrarLineaProduccion->execute();
                                    ...
                                    echo "<script>
                                            Swal.fire({
                                                icon: 'error',
                                                title: 'Error al borrar',
                                                text: 'Hubo un problema al intentar borrar la Línea de Producción',
                                                showConfirmButton: false,
                                                timer: 1500
                                            }).then(function() {
                                                window.location.href = 'indexLinea.php';
